...
export default function AdminOrdersPage() {
  ...
  const handleBulkUpdateStatus = async () => {
    ...
    try {
      setIsUpdating(true);
      const res = await adminClient.bulkUpdateOrders(Array.from(selectedOrders), { status: bulkStatus });
  ...
  const handleBulkUpdateETA = async () => {
    ...
    try {
      ...
      const res = await adminClient.bulkUpdateOrders(Array.from(selectedOrders), { estimatedDelivery: ms });
                  ...
                  {loading ? (
                    ...
                    orders.map(order => (
                      <tr key={order.publicId} className={`hover:bg-gray-50 transition-colors ${selectedOrders.has(order.publicId) ? 'bg-orange-50/50' : ''}`}>
                        ...
                        <td className="px-5 py-4">
                          <StatusControl orderId={order.publicId} currentStatus={order.status} clientType="admin" onUpdated={loadData} />
