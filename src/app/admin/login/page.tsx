...
export default function AdminLoginPage() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    setError(null);
    try {
      const res = await authService.signInWithEmail(data);
  ...
  const onGoogleLogin = async () => {
    ...
    setError(null);
    try {
      await authService.loginWithGoogle('/admin');
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <button 
          ...
          disabled={isLoading}
          className="w-full bg-black text-white py-4 rounded-xl font-bold mb-6 flex items-center justify-center gap-3 hover:bg-gray-800 active:scale-[0.98] transition-all disabled:opacity-50 shadow-md"
        >
          {isLoading ? (
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <fieldset disabled={isLoading} className="space-y-4">
            ...
            <button 
              type="submit"
              disabled={isLoading}
              className="w-full h-12 bg-gray-900 text-white rounded-xl font-bold shadow-sm disabled:opacity-70 transition-all hover:bg-black active:scale-[0.98] flex items-center justify-center mt-2"
            >
