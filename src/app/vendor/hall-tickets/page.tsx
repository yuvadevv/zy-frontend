...
import { Ticket, Search, Download, ExternalLink, Printer, Phone } from 'lucide-react';
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [data, profile] = await Promise.all([
        vendorClient.getOrders({ page: 1, limit: 100, search, status: statusTab, sort: 'newest', order_type: 'hall_ticket' }),
        vendorClient.getProfile()
      ]);
      setOrders(data.orders || []);
      setCurrentVendorId(profile.id);
    } catch (err: any) {
      setError(err.message || 'Failed to load hall ticket orders');
    } finally {
      setLoading(false);
    }
  }, [search, statusTab]);
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-20 p-4 md:p-6">
      <div className="mb-4">
        <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-gray-900 flex items-center">
          <Ticket className="w-8 h-8 mr-3 text-[#FF6B00]" /> Hall Tickets
        </h1>
        <p className="text-sm text-gray-500 mt-1">All hall ticket print orders — global vendor pool.</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        {loading && orders.length === 0 ? (
          <div className="p-12 text-center text-gray-500">Loading hall ticket orders...</div>
        ) : error ? (
          <div className="p-12 text-center text-red-500">{error}</div>
        ) : orders.length === 0 ? (
          <div className="p-12 text-center text-gray-400">No hall ticket orders found.</div>
        ) : (
                    ...
                    <div className="flex-1 space-y-3">
                      ...
                      <div className="space-y-2">
                        {(o.items || []).map((item: any, i: number) => {
                          ...
                          const docFilename = item.document_filename || item.title || 'hall_ticket.pdf';
                          return (
