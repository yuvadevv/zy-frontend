...
export default function VendorLoginPage() {
  ...
  const onGoogleLogin = async () => {
    ...
    setError(null);
    try {
      await authService.loginWithGoogle('/vendor');
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-blue-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-blue-600 font-medium text-sm">Vendor Portal Login</p>
        </div>

        {(error || urlError) && (
          <div className="mb-6 p-4 bg-red-50 text-red-600 rounded-xl text-sm text-center border border-red-100 font-medium">
            {error || urlError}
          </div>
        )}
        ...
        <button 
          ...
          disabled={isLoading}
          className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold mb-6 flex items-center justify-center gap-3 hover:bg-blue-700 active:scale-[0.98] transition-all disabled:opacity-50 shadow-md"
        >
          {isLoading ? (
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <fieldset disabled={isLoading} className="space-y-4">
            ...
            <button 
              type="submit"
              disabled={isLoading}
              className="w-full h-12 bg-blue-600 text-white rounded-xl font-bold shadow-sm disabled:opacity-70 transition-all hover:bg-blue-700 active:scale-[0.98] flex items-center justify-center mt-2"
            >
        ...
        <p className="text-center text-xs text-gray-400 mt-6">
          This portal is strictly for registered vendors.
        </p>
      </motion.div>
    </div>
  );
