      ...
      <div className="flex items-start gap-3 bg-muted/30 p-3 rounded-xl border border-border/50">
        ...
        <div className="flex flex-col gap-1">
          <span className="text-sm font-bold text-foreground flex items-center gap-2">
            {academicRecord.classrooms?.name || academicRecord.classroom_id || 'Room not set'}
            <span className="text-[10px] uppercase bg-primary text-primary-foreground px-1.5 py-0.5 rounded-sm">Default</span>
          </span>
          <span className="text-xs text-muted-foreground">
            {academicRecord.colleges?.name || 'College'}, {academicRecord.branches?.name || 'Branch'}, {academicRecord.academic_years?.name || 'Year'}, {academicRecord.blocks?.name || academicRecord.block_id || 'Block'}, Section {academicRecord.sections?.name || 'N/A'}
          </span>
